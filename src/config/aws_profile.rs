...
impl AwsProfileDetector {
    pub fn detect_profiles() -> Result<Vec<String>> {
        ...
        let contents = fs::read_to_string(&credentials_path).map_err(|e| {
            NimbusError::ConfigError(format!("Failed to read AWS credentials: {}", e))
        })?;
    ...
    fn credentials_path() -> Result<PathBuf> {
        let home = dirs::home_dir().ok_or_else(|| {
            NimbusError::ConfigError("Could not determine home directory".to_string())
        })?;
    ...
    fn parse_profile_config(contents: &str, profile: &str) -> Result<ProfileConfig> {
        ...
        if access_key_id.is_none() || secret_access_key.is_none() {
            return Err(NimbusError::ConfigError(format!(
                "Profile '{}' is missing required credentials",
                profile
            )));
        }
